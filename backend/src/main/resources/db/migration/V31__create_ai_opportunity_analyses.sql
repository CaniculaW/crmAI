create table ai_opportunity_analyses (
...
create index idx_ai_opportunity_analyses_owner_status
    on ai_opportunity_analyses (created_by, status, created_at);

create index idx_ai_opportunity_analyses_opportunity
    on ai_opportunity_analyses (created_by, opportunity_id, created_at);

insert into sys_permissions (permission_code, permission_name, permission_type, module_code, sort_order)
select 'ai.opportunity.analyze', '分析AI商机', 'operation', 'ai', 1330
where not exists (
    select 1
