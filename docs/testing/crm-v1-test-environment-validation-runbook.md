...
该命令会显示 `Ready/Pending` 行数；返回非 0 表示仍有未闭合行，需要继续补证。可随时生成项目/产品侧可填写的 Markdown 收集表：
...
该收集表用于把当前 `1-governance` 的 14 项补证要求转成项目/产品侧可填写的 Markdown 表单；它仍是收集入口，不代表正式验收通过。14 项全部 `Ready` 后，再批量写入补证模板：
...
该命令会先检查所有启动治理补证模板是否为 `Ready`，并确认具名人员、闭环值和留存证据引用完整；任一模板仍为 `Pending` 或缺证据时，命令会拒绝写入 `docs/meeting-notes/crm-kickoff-minutes.md`。
