...
要求：正式准出前，启动治理 validator 必须返回 `PASS`；若返回 `FAIL`，按输出补齐产品负责人、销售侧/管理侧业务验收人、研发负责人、前端负责人、后端负责人、测试负责人、`YYYY-MM-DD 至 YYYY-MM-DD` 上线周期、V1范围冻结状态、可留存确认/冻结证据引用或项目 `Go` 结论。
