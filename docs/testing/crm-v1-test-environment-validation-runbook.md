...
- `docs/testing/v1-uat-launch-intake.md`：用于在正式 UAT 启动前收敛具名环境、结构化 UAT窗口、证据归档位置、参与人、账号准备状态和可留存证据引用。
...
- `scripts/v1-uat-launch-intake-validate.mjs`：用于在正式 UAT 启动前校验具名环境、参与人、账号保管、结构化 UAT窗口、证据归档位置，以及启动输入证据引用是否指向 `docs/` 留存工件或外部 URL。
...
要求：正式准出前，环境证据 validator 必须返回 `PASS`；若返回 `FAIL`，按输出补齐测试环境名称、前后端地址、提交号、账号、权限样本或可留存 Smoke 证据。
...
要求：正式准出前，缺陷台账 validator 必须返回 `PASS`；若返回 `FAIL`，按输出补齐 P0/P1 汇总、缺陷明细、可留存回归证据或项目 Go/No-Go 结论。
...
| UAT启动输入 validator | `node scripts/v1-uat-launch-intake-validate.mjs docs/testing/v1-uat-launch-intake.md` 返回 PASS | 补齐具名环境、`YYYY-MM-DD HH:mm 至 YYYY-MM-DD HH:mm` UAT窗口、证据归档位置、参与人、账号保管证据或可留存证据引用 |
