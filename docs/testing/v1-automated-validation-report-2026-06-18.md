...
| 启动治理准出校验 | 已提供 `scripts/v1-kickoff-governance-validate.mjs` 和 `docs/meeting-notes/crm-kickoff-minutes.md`，校验负责人、业务验收人、`YYYY-MM-DD 至 YYYY-MM-DD` 上线周期、V1范围冻结、确认/冻结证据引用必须指向 `docs/` 留存工件或外部 URL、范围边界、项目 `Go` 结论和敏感材料 | 通过 |
...
| `node --test scripts/*.test.mjs` | 208 tests passed |
| `node --test scripts/v1-kickoff-governance-validate.test.mjs` | 7 tests passed；覆盖完整启动治理记录、当前草稿 No-Go、缺失负责人、V2/AI 范围误入 V1、上线周期非结构化、确认/冻结证据引用未指向可留存工件/URL和敏感材料拦截 |
