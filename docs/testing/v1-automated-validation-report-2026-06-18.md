...
| UAT证据包准出校验 | 已提供 `scripts/v1-uat-evidence-pack-validate.mjs`，校验 Go/No-Go、P0/P1缺陷、UAT用例、自动化结果、签署记录一致性、验收日期/前后端地址/Git 提交号格式、候选版本/前端构建/后端构建/数据库版本必填、基本信息责任人必填且具名、已通过 UAT 用例验收人具名、已同意签署人具名、已同意签署日期格式，以及通过项证据必须指向 `docs/` 留存工件或外部 URL | 通过 |
...
| `node --test scripts/*.test.mjs` | 234 tests passed |
...
| `node --test scripts/v1-uat-evidence-pack-validate.test.mjs` | 12 tests passed；覆盖证据包验收日期、前后端地址、Git 提交号和已同意签署日期必须结构化，候选版本、前端构建、后端构建和数据库版本必须补齐，已通过自动化、环境、UAT用例和签署证据必须指向可留存仓库工件或外部 URL，且基本信息责任人必须必填并不得以角色标签替代姓名，已通过 UAT 用例验收人和已同意签署人不得以角色标签替代姓名 |
