...
| `node --test scripts/*.test.mjs` | 130 tests passed |
...
| `node --test scripts/v1-uat-readiness-check.test.mjs` | 46 tests passed；包含启动治理纪要、UAT执行派工追踪表、UAT启动输入、UAT具名环境证据、UAT逐项执行包、UAT证据清单、UAT缺陷台账、UAT签署台账、tracker validator gate、聚合状态报告gate、UAT行动计划gate、Go/No-Go会议包gate、生成文档一致性gate、计划状态一致性gate、验收清单一致性gate、UAT覆盖gate、验证追踪矩阵gate、阻塞项一致性gate和证据秘密扫描gate |
| `node --test ../scripts/v1-uat-readiness-check.test.mjs` | 46 tests passed；覆盖CI前端job相对路径 |
...
| `node --test scripts/v1-blocker-consistency-check.test.mjs` | 3 tests passed；覆盖决策材料和执行包完整映射当前 release gate 阻塞项、决策文档漏项和执行行动项漏项 |
...
| `node --test scripts/v1-secret-scan-check.test.mjs` | 3 tests passed；覆盖脱敏占位允许、明文密码拦截和 Bearer token 拦截 |
