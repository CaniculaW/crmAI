...
| `node --test scripts/*.test.mjs` | 315 tests passed |
...
| `node --test scripts/v1-external-uat-request.test.mjs` | 11 tests passed；覆盖 No-Go 外部 UAT 请求包、按责任侧分组的关闭台账、映射 manifest Evidence ID 的证据收件清单、机器可读外部 UAT 阻塞项 JSON、机器可读最终门禁命令、绝对路径 UAT 源文档、全量 Go 状态，以及 validator blocker 仍存在时请求包、阻塞项 JSON 和关闭台账不得输出关闭/Go 状态 |
