...
| `node --test scripts/*.test.mjs` | 317 tests passed |
...
| `node --test scripts/v1-final-evidence-handoff-check.test.mjs` | 10 tests passed；覆盖 No-Go 阻塞和门禁命令可见、No-Go 时误称验收通过、缺失最终门禁命令、缺失机器可读最终门禁命令、缺失验收/覆盖/追踪/交接命令、缺失生成的UAT交接包、缺失外部UAT关闭台账生成命令、缺失外部UAT证据收件清单生成命令、缺失外部UAT阻塞项JSON生成命令和隐藏外部阻塞项 |
