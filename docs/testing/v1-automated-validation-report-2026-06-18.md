...
| `node --test scripts/*.test.mjs` | 337 tests passed |
...
| `node --test scripts/v1-uat-readiness-check.test.mjs` | 136 tests passed；包含启动治理纪要具名负责人、启动治理纪要可留存证据引用、启动治理纪要 `docs/` 工件存在性、启动治理上线周期格式、UAT执行派工追踪表具名角色负责人、UAT执行派工追踪表具名 UAT 用例验收人、UAT执行派工追踪表可留存证据引用、UAT执行派工追踪表 `docs/` 工件存在性、UAT启动输入具名参与人、UAT启动输入具名账号保管 Owner、UAT启动输入可留存证据引用、UAT启动输入 `docs/` 工件存在性、UAT启动输入 URL/提交号格式、结构化UAT窗口、UAT具名环境 URL/提交号格式、UAT具名环境PASS检查Owner、UAT具名环境可留存证据、UAT具名环境 `docs/` 工件存在性、UAT证据包基本信息格式、UAT证据包版本字段必填、UAT证据包基本信息责任人必填、UAT证据包基本信息责任人具名、UAT证据包已通过 UAT 用例验收人具名、UAT证据包已同意签署人具名、UAT证据包已同意签署日期格式、UAT证据包敏感材料拦截、UAT证据包通过项可留存证据引用、UAT证据包 `docs/` 工件存在性、UAT证据清单 PASS Owner 具名、UAT证据清单通过项可留存证据引用、UAT缺陷台账来源用例格式、UAT缺陷台账具名缺陷 Owner、UAT缺陷台账 `docs/` 工件存在性、UAT逐项执行包、UAT证据清单、证据引用保全检查、UAT缺陷台账可留存回归证据、UAT签署台账具名签署人、UAT签署台账可留存证据引用、UAT签署台账 `docs/` 工件存在性、release gate JSON 快照、release gate JSON schema gate、release gate JSON check id 唯一性 gate、release gate JSON 未知 check id 拦截 gate、release gate JSON decision consistency gate、release gate JSON live alignment gate、tracker validator gate、聚合状态报告gate、聚合状态报告机器可读门禁命令gate、UAT行动计划gate、UAT行动计划机器可读门禁命令gate、UAT逐项执行包机器可读门禁命令gate、测试环境Runbook机器可读最终门禁命令gate、Go/No-Go会议包gate、Go/No-Go会议包机器可读门禁命令gate、外部UAT请求包gate、外部UAT请求包机器可读门禁命令gate、外部UAT请求包稳定阻塞项ID覆盖gate、外部UAT请求包关闭顺序覆盖gate、外部UAT请求包阶段汇总覆盖gate、外部UAT请求包下一关闭阶段交接覆盖gate、外部UAT请求包矛盾Go阻塞项覆盖gate、外部UAT证据收件清单矛盾Go阻塞项覆盖gate、外部UAT关闭台账gate、外部UAT关闭台账Open数量一致性gate、外部UAT关闭台账下一关闭阶段交接gate、外部UAT证据收件清单gate、外部UAT证据收件清单manifest ID单一归属gate、外部UAT证据收件清单manifest ID文档唯一性gate、生成文档一致性gate、生成状态报告提交绑定gate、CI checkout 历史深度gate、CI全量脚本测试兜底gate、外部UAT阻塞项JSON gate、外部UAT阻塞项JSON汇总一致性gate、外部UAT阻塞项JSON稳定ID gate、外部UAT阻塞项JSON关闭阶段汇总gate、外部UAT阻塞项JSON有序阶段明细gate、外部UAT阻塞项JSON下一关闭阶段交接gate、外部UAT阻塞项JSON去重gate、计划状态一致性gate、验收清单一致性gate、UAT覆盖gate、UAT执行明细gate、验证追踪矩阵gate、阻塞项一致性gate、外部UAT请求覆盖gate、外部UAT请求责任侧路由gate、外部UAT请求机器可读门禁命令覆盖gate、最终交接证据一致性gate、最终交接关键验证命令覆盖gate、最终交接机器可读门禁命令覆盖gate、最终交接状态与外部阻塞可见性gate、最终交接外部UAT关闭态拦截gate、证据秘密扫描gate、README 交接入口秘密扫描覆盖gate、README 外部UAT生成命令覆盖gate、README 秘密扫描入口和 README 关键准出检查入口 |
| `node --test ../scripts/v1-uat-readiness-check.test.mjs` | 136 tests passed；覆盖CI前端job相对路径 |
...
| `node --test scripts/v1-validation-status.test.mjs` | 4 tests passed；覆盖 No-Go 聚合状态、机器可读最终门禁命令、绝对路径 UAT 源文档、全量 Go 状态和显式 git commit 参数 |
...
| `node --test scripts/v1-uat-action-plan.test.mjs` | 3 tests passed；覆盖 No-Go 行动计划、机器可读最终门禁命令、绝对路径 UAT 源文档和全量 Go 状态 |
| `node scripts/v1-uat-action-plan.mjs --output docs/testing/v1-uat-action-plan.md` | 生成当前 `No-Go` UAT行动计划 |
| `node --test scripts/v1-uat-execution-pack.test.mjs` | 3 tests passed；覆盖 No-Go 逐项补证清单、机器可读最终门禁命令、绝对路径 UAT 源文档和全量 Go 状态 |
...
| `node --test scripts/v1-external-uat-request.test.mjs` | 18 tests passed；覆盖 No-Go 外部 UAT 请求包、按责任侧分组的关闭台账、映射 manifest Evidence ID 的证据收件清单、下一闭环阶段交接包、带稳定 `blockerId`、关闭阶段、关闭顺序、有序阶段汇总和下一关闭阶段交接信息的机器可读外部 UAT 阻塞项 JSON、机器可读最终门禁命令、绝对路径 UAT 源文档、全量 Go 状态，以及 validator blocker 仍存在时请求包、阻塞项 JSON、关闭台账和证据收件清单不得输出关闭/Go 状态 |
| `node scripts/v1-external-uat-request.mjs --output docs/testing/v1-external-uat-request.md` | 生成当前 `No-Go` 外部 UAT 请求包 |
| `node scripts/v1-external-uat-request.mjs --closure-checklist --output docs/testing/v1-external-uat-closure-checklist.md` | 生成当前 `No-Go` 外部 UAT 关闭台账 |
| `node scripts/v1-external-uat-request.mjs --evidence-intake --output docs/testing/v1-external-uat-evidence-intake.md` | 生成当前 `No-Go` 外部 UAT 证据收件清单 |
| `node scripts/v1-external-uat-request.mjs --next-closure-phase --output docs/testing/v1-next-closure-phase.md` | 生成当前 `No-Go` 下一闭环阶段交接包 |
| `node scripts/v1-external-uat-request.mjs --json --output docs/testing/v1-external-uat-blockers.json` | 生成当前 `No-Go` 外部 UAT 阻塞项 JSON，并输出稳定 `blockerId`、关闭阶段、关闭顺序、有序阶段汇总和下一关闭阶段交接信息 |
| `node --test scripts/v1-generated-docs-check.test.mjs` | 9 tests passed；覆盖生成文档一致、生成器输出漂移、release gate JSON 快照缺失、外部 UAT 阻塞项 JSON 快照缺失、外部 UAT 关闭台账缺失、外部 UAT 证据收件清单缺失、下一闭环阶段交接包缺失、状态报告提交绑定陈旧和提交后直接父提交复用 |
| `node scripts/v1-generated-docs-check.mjs` | V1 generated docs check passed；状态报告、行动计划、逐项执行包、会议包、外部 UAT 请求包、外部 UAT 关闭台账、外部 UAT 证据收件清单、下一闭环阶段交接包、外部 UAT 阻塞项 JSON 和 release gate JSON 快照与当前生成器一致，且状态报告绑定当前或直接上一提交 |
| `node --test scripts/v1-release-gate-status-check.test.mjs` | 8 tests passed；覆盖 JSON schema 稳定、非法 JSON、缺失必需 release gate check id、重复 release gate check id、未知 release gate check id、result/ok 不一致、result/ok/decision 不一致和 JSON 快照未对齐实时 release gate |
| `node scripts/v1-release-gate-status-check.mjs` | V1 release gate status JSON check passed；当前 release gate JSON 快照字段、值域、check id 唯一性、已知 check id 合约、result/ok/decision 一致性和实时 release gate 对齐稳定 |
...
| `node --test scripts/v1-acceptance-checklist-check.test.mjs` | 4 tests passed；覆盖待业务验收状态和 release gate No-Go 一致、误标业务通过时失败、当前外部 UAT/最终交接工件可见，以及缺失交接工件时失败 |
...
| `node --test scripts/v1-external-uat-request-coverage-check.test.mjs` | 6 tests passed；覆盖外部 UAT 请求包阻塞项完整、责任侧路由、漏写 validator 失败项、漏写命令、漏写机器可读最终门禁命令和 No-Go 时误标请求关闭 |
| `node scripts/v1-external-uat-request-coverage-check.mjs` | V1 external UAT request coverage check passed；外部 UAT 请求包覆盖当前失败 validator 明细、补证命令、机器可读最终门禁命令和责任侧路由 |
| `node --test scripts/v1-final-evidence-handoff-check.test.mjs` | 12 tests passed；覆盖 No-Go 阻塞和门禁命令可见、No-Go 时误称验收通过、缺失最终门禁命令、缺失机器可读最终门禁命令、缺失验收/覆盖/追踪/交接命令、缺失生成的UAT交接包、缺失外部UAT关闭台账生成命令、缺失外部UAT证据收件清单生成命令、缺失下一闭环阶段交接包生成命令、缺失外部UAT阻塞项JSON生成命令、隐藏外部阻塞项和 No-Go 时外部UAT交接包误标关闭 |
...
| `node --test scripts/v1-secret-scan-check.test.mjs` | 10 tests passed；覆盖当前证据清单包含 README 交接入口、外部 UAT 请求包、外部 UAT 关闭台账、外部 UAT 证据收件清单、下一闭环阶段交接包、外部 UAT 阻塞项 JSON、release gate JSON 快照、脱敏占位允许、明文密码拦截和 Bearer token 拦截 |
...
聚合状态报告见 `docs/testing/v1-validation-status.md`；UAT行动计划见 `docs/testing/v1-uat-action-plan.md`，按项目/产品、测试、业务UAT和研发拆分下一步；UAT逐项执行包见 `docs/testing/v1-uat-execution-pack.md`，把失败门禁拆成 KICKOFF、ENV、PRE、SMK、UAT、DEF、SIGNOFF 和 GO-NOGO 补证项；Go/No-Go会议包见 `docs/testing/v1-go-no-go-meeting.md`，用于正式准出会议留痕；外部UAT请求包见 `docs/testing/v1-external-uat-request.md`，面向项目/产品、测试、业务UAT和研发集中列出源文档、验证命令和当前阻塞请求；外部UAT关闭台账见 `docs/testing/v1-external-uat-closure-checklist.md`，按责任侧列出每个 open blocker 的源文档、验证命令和关闭证据要求；外部UAT证据收件清单见 `docs/testing/v1-external-uat-evidence-intake.md`，把外部补证工作映射到 manifest Evidence ID、正式源文档和验证命令；下一闭环阶段交接包见 `docs/testing/v1-next-closure-phase.md`，单独列出当前最早开放闭环阶段的 `blockerId`、责任侧、源文档、验证命令和关闭证据要求；外部UAT阻塞项JSON见 `docs/testing/v1-external-uat-blockers.json`，按稳定 `blockerId`、责任侧、关闭阶段、关闭顺序、有序阶段汇总、下一关闭阶段交接信息、来源 gate、check id、源文档和验证命令提供机器可读阻塞项；release gate JSON 快照见 `docs/testing/v1-release-gate-status.json`，用于看板或验收机器人读取当前准出状态，并由 `scripts/v1-release-gate-status-check.mjs` 校验字段、值域、check id 唯一性、已知 check id 合约、result/ok/decision 一致性和实时 release gate 对齐稳定。

生成文档一致性检查见 `scripts/v1-generated-docs-check.mjs`，用于防止生成器、状态报告提交绑定和已提交证据文档漂移；release gate JSON schema 校验见 `scripts/v1-release-gate-status-check.mjs`，用于防止机器可读准出快照结构漂移、重复或未知门禁项、result/ok/decision 决策矛盾和快照滞后；计划状态一致性检查见 `scripts/v1-plan-status-check.mjs`，用于防止启动计划未完成项被误配为 V1 Go；验收清单一致性检查见 `scripts/v1-acceptance-checklist-check.mjs`，用于防止 AC-001 至 AC-017 在最终放行仍为 No-Go 时被误标为业务通过，并要求外部 UAT/最终交接工件在验收清单中保持可见；UAT覆盖检查见 `scripts/v1-uat-coverage-check.mjs`，用于防止 UAT-001 至 UAT-010 漏覆盖任一 V1 验收项，且防止 UAT 行缺操作人或证据要求；验证追踪矩阵一致性检查见 `scripts/v1-traceability-check.mjs`，用于防止 AC-001 至 AC-017 的研发证据矩阵漏项或误标项目验收通过；阻塞项一致性检查见 `scripts/v1-blocker-consistency-check.mjs`，用于防止当前 release gate 阻塞项从状态报告、行动计划、会议包或执行行动项中漏写；外部UAT请求覆盖检查见 `scripts/v1-external-uat-request-coverage-check.mjs`，用于防止项目/测试/业务侧请求包漏掉当前失败 validator 明细、补证命令或责任侧路由；最终交接证据一致性检查见 `scripts/v1-final-evidence-handoff-check.mjs`，用于防止 README、候选版本记录、自动化验证报告、Runbook、验收清单、UAT行动计划、UAT逐项执行包、Go/No-Go会议包、外部UAT请求包、外部UAT关闭台账、外部UAT证据收件清单或下一闭环阶段交接包在 `No-Go` 状态下缺失、漏掉最终门禁命令、隐藏阻塞项，或误称 V1 已验收通过/可正式发布；证据秘密扫描见 `scripts/v1-secret-scan-check.mjs`，用于防止当前 V1 证据文档、外部 UAT 请求包、外部 UAT 关闭台账、外部 UAT 证据收件清单、下一闭环阶段交接包、外部 UAT 阻塞项 JSON 和 release gate JSON 快照中出现明文密码、Bearer token、API key 或等价敏感材料。rc.8 UAT 交接草稿见 `docs/testing/evidence/crm-v1-uat-evidence-pack-rc8-draft.md`。该草稿已预填工程侧自动化和本地验证证据，validator 实测为 `FAIL / No-Go`，最终 V1 放行门禁也返回 `FAIL`，仍需项目/测试/业务侧补齐启动治理、具名测试环境账号、UAT-001 至 UAT-010、缺陷汇总、回归证据和签署台账；readiness 审计会防止该草稿在外部 UAT 完成前被误标为 Go/PASS。
