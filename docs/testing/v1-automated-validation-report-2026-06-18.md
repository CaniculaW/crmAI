...
| `node --test scripts/*.test.mjs` | 248 tests passed |
...
| `node --test scripts/v1-release-gate.test.mjs` | 19 tests passed；覆盖完整 Go、从已填写 UAT 源文件读取完整 Go、CLI 命名外部 UAT 源文档参数、机器可读 JSON 输出、readiness 失败、启动治理未完成、UAT启动输入未完成、UAT具名环境未完成、No-Go、正式UAT证据包缺失本地 `docs/` 工件、UAT具名环境缺失本地 `docs/` 工件、签署台账缺失本地 `docs/` 工件、缺陷台账缺失本地 `docs/` 工件、Conditional Go、tracker 未完成、证据清单未完成、证据引用缺失、缺陷台账未完成和签署台账未完成 |
