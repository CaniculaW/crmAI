...
| 测试环境可用 | 测试环境可访问，前后端版本部署完成，基础配置可维护；本地 PostgreSQL 部署态和浏览器 Smoke 已通过 | 研发/运维 | 本地通过，测试环境待确认 |
...
| 本地部署态登录和系统页Smoke | 已通过浏览器Smoke Test | PostgreSQL 16 + Spring Boot + Vite dev proxy；`demo_admin` 登录后系统管理页展示演示组织、用户、角色；console 0 warning/error |
| 部署态API bootstrap探针 | 已通过API Smoke Test | `POST /api/auth/login` + `GET /api/bootstrap` 返回 200，`permissions_count = 25` |
