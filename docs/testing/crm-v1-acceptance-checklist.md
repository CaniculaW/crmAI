...
| V1生成文档一致性检查 | 已形成 | `scripts/v1-generated-docs-check.mjs` 重放状态报告、行动计划、逐项执行包、会议包、外部UAT请求包、关闭台账、证据收件清单、阻塞项JSON和 release gate JSON 快照，防止生成文档漂移 |
| V1最终交接证据一致性检查 | 已形成 | `scripts/v1-final-evidence-handoff-check.mjs` 校验 README、候选版本记录、自动化验证报告、Runbook、验收清单、UAT行动计划、UAT逐项执行包、Go/No-Go会议包、外部UAT请求包、外部UAT关闭台账、外部UAT证据收件清单和外部UAT阻塞项JSON在 release gate `No-Go` 时继续保留最终门禁命令、外部 UAT/签署阻塞项，且不得误称 V1 已验收通过或可正式发布；配套 `scripts/v1-final-evidence-handoff-check.test.mjs` |
| V1证据秘密扫描 | 已形成 | `scripts/v1-secret-scan-check.mjs` 扫描当前 V1 验收证据文档、外部UAT请求包、外部UAT关闭台账、外部UAT证据收件清单、外部UAT阻塞项JSON和 release gate JSON 快照，拦截明文密码、Bearer token、API key 或等价敏感材料；配套 `scripts/v1-secret-scan-check.test.mjs` |
