...
| 证据包准出校验 | UAT 证据包填写完成后执行 `node scripts/v1-uat-evidence-pack-validate.mjs <证据包>`；若选择 Go，validator 必须返回 PASS；验收日期、已同意签署日期、前后端地址和 Git 提交号必须结构化，基本信息责任人、已通过 UAT 用例验收人和已同意签署人必须必填且具名，且通过项证据必须指向可留存工件或外部 URL |
...
| UAT证据包准出校验器 | 已形成 | `scripts/v1-uat-evidence-pack-validate.mjs` 读取已填写 UAT 证据包，校验自动化结果、环境 Smoke、UAT-001 至 UAT-010、P0/P1缺陷、Go/No-Go判定、签署记录一致性、验收日期/前后端地址/Git 提交号格式、基本信息责任人必填且具名、已通过 UAT 用例验收人具名、已同意签署人具名、已同意签署日期格式和通过项可留存证据引用；配套 `scripts/v1-uat-evidence-pack-validate.test.mjs` |
