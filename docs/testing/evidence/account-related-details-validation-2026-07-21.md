...
- 前端全量测试：`npm --prefix frontend test`，`9/9` 个测试文件、`192/192` 个测试通过，退出码 `0`。
- 前端生产构建：`npm --prefix frontend run build`，TypeScript 与 Vite 构建通过，转换 `3216` 个模块，退出码 `0`。
- 静态检查：`git diff --check` 无输出。
- 已知非阻塞提示：JSDOM 不实现伪元素 `getComputedStyle`；Vite 提示 Ant Design vendor chunk 超过 `500 kB`。
