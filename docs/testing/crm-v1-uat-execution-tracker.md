...
- 最终放行门禁：`node scripts/v1-release-gate.mjs . <crm-v1-uat-evidence-pack.md>`
...
- V1最终验证通过必须执行 `node scripts/v1-release-gate.mjs . <crm-v1-uat-evidence-pack.md>` 并返回 `PASS`。
...
| V1最终放行门禁 | `node scripts/v1-release-gate.mjs . <crm-v1-uat-evidence-pack.md>` | 返回 `PASS`，且项目负责人选择 `Go` | 当前rc.8草稿为 `FAIL / No-Go` |
