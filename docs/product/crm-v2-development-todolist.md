...
|---:|---|---|---|---|---|
| 1 | Current | V2 启动与 UI/范围对齐 | V2 总导航、模块边界、页面逻辑、验收口径 | 确认 V2 从商机成交到核销的业务链路、页面层级和 TODO 机制 | V2 设计说明、模块级 TODOList、模块 2 Current、无范围歧义 |
| 2 | Current | 方案与标书链路 | 方案/标书列表、详情、新建/编辑、版本、报价/测算、自评、附件增删下载 | 让商业方案和商业谈判阶段有材料、报价、评审、风险和附件归档入口 | 商机可进入方案/标书；可维护方案版本、报价测算、标书自评；附件可新增、删除、下载；风险可回写商机 |
...
当前任务：`v2-solution-documents-design`
...
| 方案与标书链路 | Current | - | - | - | - | 附件模式已纳入设计；实施计划已创建，待执行 |
| 合同管理链路 | Pending | - | - | - | - | - |
| 开票管理链路 | Pending | - | - | - | - | - |
| 回款管理链路 | Pending | - | - | - | - | - |
| 发票回款核销链路 | Pending | - | - | - | - | - |
...
- 已完成：1
- 当前模块：2. 方案与标书链路
- 当前步骤：Step 9. 按实施计划进入后端数据模型与权限实现
- 当前 TODO：先写迁移测试，再新增 `crm_solution_documents` 和 `solution.*` 权限点
- 完成标准：后端迁移测试红绿通过，进入方案标书 API 实现
```
...
| 5 | Pending | 后端实现与测试 | Migration、Service、Controller、权限测试通过 |
| 6 | Pending | 前端实现与测试 | 菜单、页面、表单、详情、状态文案测试通过 |
| 7 | Pending | UAT 验收 | `/solutions` 关键链路与附件新增、删除、下载无服务端异常、无控制台应用错误 |
| 8 | Pending | 提交切换 | 模块 2 Done，模块 3 合同管理 Current |
