...
| 5 | Current | 回款管理链路 | 回款计划、回款流水、到账确认、逾期、回款跟进 | 管理合同应收、实收、逾期和回款责任 | 回款计划与合同联动；流水可登记；逾期可识别；回款跟进可追溯 |
...
当前任务：`v2-receivable-management-implementation`
...
- [ ] Step 11：按实施计划进入后端数据模型、回款计划 API 和到账流水 API 实现。

当前模块不做：

- 不直接开始写回款/核销代码。
- 不把驾驶舱和 AI 助手并入 V2。
- 不接入税控、电子发票平台、财务总账和复杂审批流。
...
| 回款管理链路 | Current | - | - | - | - | 待启动模块 5 设计与实施计划 |
| 发票回款核销链路 | Pending | - | - | - | - | - |
...
- 已完成：4
- 当前模块：5. 回款管理链路
- 当前步骤：Step 11. 按实施计划进入后端数据模型与 API 实现
- 当前 TODO：执行 `docs/superpowers/plans/2026-06-30-v2-receivable-management.md` 的 Task 1-4
- 完成标准：V18 迁移、回款计划 API、到账流水 API、附件对象扩展通过后端定向验证
```
