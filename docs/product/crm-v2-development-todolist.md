...
当前任务：`v2-receivable-management-design`
...
- [ ] Step 10：启动模块 5 回款管理，输出页面逻辑、数据模型、API 和实施计划。
...
- 已完成：4
- 当前模块：5. 回款管理链路
- 当前步骤：Step 10. 模块 5 启动与方案设计
- 当前 TODO：输出回款管理页面逻辑、数据模型、API 契约和 TDD 实施计划
- 完成标准：回款计划、回款流水、到账确认、逾期和回款跟进范围清晰，可进入实现
```
