...
- [ ] Step 29：建设 V2 专用浏览器 E2E、角色矩阵、移动端和附件真实上传下载证据，作为生产发布复审门禁。
  ...
  - [ ] Step 29.2：补齐角色权限矩阵验收，覆盖销售、商务/法务、财务、管理层、低权限用户。
  - [ ] Step 29.3：补齐移动端/平板响应式证据，重点复核遮挡、换行、表格可读性和关键操作入口。
  - [ ] Step 29.4：补齐真实附件上传/下载能力或调整验收口径，明确 URL 元数据管理与文件存储边界。
...
- V2 专用浏览器 smoke 已建设并通过：`npm run smoke:v2:browser` 覆盖 18 条路由 × desktop/mobile 两种视口，console failure = 0，API failed response = 0，证据目录 `docs/testing/evidence/artifacts/v2-browser-smoke-20260702/`。
...
- 已完成：9
- 当前模块：V2 生产发布复审证据门禁
- 当前步骤：Step 29.2 角色权限矩阵验收
- 当前 TODO：补齐角色矩阵、移动端/平板响应式、真实附件上传下载、性能/安全证据
- 完成标准：生产发布复审通过
```
