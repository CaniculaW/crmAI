...
| 4 | Current | 开票管理链路 | 开票计划、开票申请、发票登记、异常、签收、作废 | 管理合同下应该开多少、何时开、实际开了多少、异常是什么 | 开票总额不超过合同金额；已开票可进入核销；异常/作废可追踪 |
...
当前任务：`v2-invoice-management-kickoff`
...
- [ ] Step 3：启动模块 4 开票管理，输出模块 TODO、页面逻辑、业务边界和验收标准。
- [ ] Step 4：确认开票管理推荐方案后进入数据模型与 API 契约设计。
...
- 模块 4 开票管理输出页面逻辑与实施 TODO 后进入 TDD 实现。
...
- 已完成：3
- 当前模块：4. 开票管理链路
- 当前步骤：Step 1. 模块启动与业务/UI 对齐
- 当前 TODO：输出开票计划、开票申请、发票登记、异常、签收、作废的页面逻辑与模块 TODO
- 完成标准：开票管理范围、页面结构、状态流转、金额校验、附件/审计点和验收方式明确
```
