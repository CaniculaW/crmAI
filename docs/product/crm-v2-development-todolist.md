...
- [ ] Step 1：读取模块 3 合同管理范围、V2 已有链路和模块 2 输出。
- [ ] Step 2：输出合同管理页面逻辑与业务/UI 方案。
- [ ] Step 3：设计合同数据模型、状态、金额口径和审计点。
- [ ] Step 4：设计合同 API 契约。
- [ ] Step 5：创建模块 3 实施计划。
...
- 已完成：2
- 当前模块：3. 合同管理链路
- 当前步骤：Step 1. 合同管理模块启动
- 当前 TODO：读取模块 3 范围，输出业务/UI 方案与实施计划
- 完成标准：合同管理设计说明与实施计划完成，进入后端数据模型实现
```
