...
| 2 | Current | 指标口径与数据权限 | 统计维度、金额口径、时间口径、组织/角色数据范围 | 让管理指标能被解释、被验证、被权限控制 | 指标字典、权限规则、API 测试设计完成 |
...
当前任务：`v3-metrics-permission-design`
...
- [ ] Step 7：沈思维确认 V3 指标字典、权限口径和验收方式。
- [ ] Step 8：进入 V3 经营驾驶舱总览模块。

完成标准：

- 指标口径可解释、可测试。
- 数据权限沿用现有 own、department、department_tree、global、collaborated 机制。
- 经营总览首版所需指标字段清楚。
- API 测试和浏览器验收口径清楚。
...
- 已完成：1
- 当前模块：指标口径与数据权限
- 当前步骤：Step 6 输出 V3 指标口径与数据权限设计说明
- 当前 TODO：等待沈思维确认 V3 指标字典、权限口径和验收方式
- 完成标准：指标口径可解释、可测试、可按数据权限过滤
- 本轮预计产出：指标字典、权限口径、API 验收口径、模块切换记录

上一模块：
- 状态：V3 启动与 UI/范围对齐 Done
- 验证结果：沈思维已确认推荐方案 B，模块顺序通过
- 提交号：f6016cc
- 遗留问题：无
```
