...
通过项证据需引用 `docs/` 下留存文件或外部 `http(s)` 系统链接，不能只填写游离文件名或文本说明。验收日期和已同意签署日期必须为 `YYYY-MM-DD`，前端访问地址和后端 API 地址必须为 `http(s)` URL，Git 提交号必须为40位提交 SHA。基本信息责任人、已通过 UAT 用例验收人和已同意签署人必须填写具体姓名，不得缺失、留空或用“测试负责人”“销售侧验收人”“Owner”等角色标签替代。
