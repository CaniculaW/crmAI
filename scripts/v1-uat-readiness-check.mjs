...
export function evaluateReadinessSnapshot(snapshot) {
  ...
  checks.push(makeCheck(
    "v1-progress-todo-generator",
    includesAll(workflow + progressTodoGenerator + progressTodoGeneratorTest, [
      "node --test scripts/v1-progress-todo.test.mjs",
      "node scripts/v1-progress-todo.mjs --output docs/testing/v1-progress-todo.md",
      "generateV1ProgressTodoMarkdown",
  ...
  checks.push(makeCheck(
    ...
    "V1 release gate JSON status checker is tested and wired into CI to keep the machine-readable release gate snapshot schema, check-id uniqueness, known check-id contract, result/decision consistency, and live release-gate alignment stable."
  ));
  ...
  checks.push(makeCheck(
    ...
    "V1 progress TODO board inventories total blockers, current task, phase TODOList, task-switch display rules, and validation commands."
  ));
  ...
  checks.push(makeCheck(
    ...
    "README links the test environment, V1 RC record, external UAT generated handoff commands, and final verification entrypoints."
  ));
