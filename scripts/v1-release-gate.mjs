...
export function evaluateV1ReleaseGate({
  ...
  const checks = [
    ...
    makeCheck(
      ...
      kickoffResult.ok,
      kickoffResult.ok
        ? "Kickoff governance and V1 scope freeze are complete."
        : `Kickoff governance failed: ${kickoffResult.failed.map((check) => check.id).join(", ")}`
    ),
    makeCheck(
      "uat-launch-intake",
...
export function evaluateV1ReleaseGateFromFiles(
  ...
  const environmentResult = evaluateUatEnvironmentEvidence(
    readFileSync(absoluteEnvironmentPath, "utf8"),
    { rootDir }
  );
  ...
  const trackerResult = evaluateUatExecutionTracker(
    readFileSync(absoluteTrackerPath, "utf8"),
    { rootDir }
  );
