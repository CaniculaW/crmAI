...
function commandList(evidencePath = DEFAULT_EVIDENCE_PATH, trackerPath = DEFAULT_TRACKER_PATH) {
  return [
...
export function generateV1ValidationStatusMarkdown({
  ...
  for (const command of commandList(evidencePath, trackerPath)) {
    lines.push(`- \`${command}\``);
  }
  ...
  lines.push("");
  lines.push("V1验证通过必须同时满足：readiness PASS、UAT证据包 validator PASS、UAT执行追踪表 validator PASS、最终 release gate PASS，且项目负责人结论为 `Go`。");
