...
test("fails when a confirmed kickoff owner is only a role label", () => {
  const roleLabelOwner = completeKickoff.replace(
    "| 产品负责人 | Wang Qiang | 已确认 | docs/meeting-notes/evidence/kickoff/product-owner.md | 负责需求冻结、范围管理和业务验收协调 |",
    "| 产品负责人 | 产品负责人 | 已确认 | docs/meeting-notes/evidence/kickoff/product-owner.md | 负责需求冻结、范围管理和业务验收协调 |"
  );

  const result = evaluateKickoffGovernance(roleLabelOwner);

  assert.equal(result.ok, false);
  assert.deepEqual(result.invalidOwnerNameRoles, ["产品负责人"]);
  assert.ok(result.failed.some((check) => check.id === "owner-name-format"));
});
