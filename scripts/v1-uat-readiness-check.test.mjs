...
jobs:
  deployment-config:
    steps:
      ...
      - run: node scripts/v1-kickoff-governance-evidence-intake.mjs --input docs/meeting-notes/evidence/kickoff/intake.json --collection-form --output docs/meeting-notes/evidence/kickoff/intake-collection-form.md
      ...
      - run: node --test scripts/v1-progress-todo.test.mjs
      - run: node scripts/v1-progress-todo.mjs --output docs/testing/v1-progress-todo.md
...
node scripts/v1-release-gate.mjs --json
`,
  "docs/meeting-notes/evidence/kickoff/closure-evidence-pack.md": `CRM V1 Kickoff Governance Evidence Pack
Current task: \`1-governance\`
Current owner side: 项目/产品
Open kickoff blockers: 4
Target source document: \`docs/meeting-notes/crm-kickoff-minutes.md\`
Decision target: \`Go\`
Do not record plaintext passwords
Owner Confirmation TODOList
...
test("accepts kickoff governance after Ready evidence is applied", () => {
  const readyEvidence = "CRM V1 Kickoff Governance Evidence - 产品负责人\nEvidence type: `owner`\nEvidence status: `Ready`\nEvidence Intake\nRetained evidence reference | docs/meeting-notes/evidence/kickoff/product-owner.md |\nDo not record plaintext passwords\n";
  const snapshot = {
    ...completeSnapshot,
    "docs/meeting-notes/crm-kickoff-minutes.md": completeSnapshot["docs/meeting-notes/crm-kickoff-minutes.md"].replace("Decision: No-Go", "Decision: Go")
  };
  ...
  for (const path of [
    ...
    snapshot[path] = readyEvidence;
  }

  const result = evaluateReadinessSnapshot(snapshot);

  assert.equal(result.failed.some((check) => check.id === "kickoff-governance"), false);
  assert.equal(result.failed.some((check) => check.id === "kickoff-governance-evidence-scaffold-docs"), false);
});
...
test("fails when the external UAT blockers JSON omits stable blocker ids", () => {
  const payload = JSON.parse(completeSnapshot["docs/testing/v1-external-uat-blockers.json"]);
  delete payload.blockers[0].blockerId;
...
test("fails when the external UAT blockers JSON omits closure sequencing", () => {
  const payload = JSON.parse(completeSnapshot["docs/testing/v1-external-uat-blockers.json"]);
  delete payload.summary.byClosurePhase;
  delete payload.blockers[0].closurePhase;
  delete payload.blockers[0].closureOrder;
...
test("fails when the external UAT blockers JSON omits ordered closure phase summaries", () => {
  const payload = JSON.parse(completeSnapshot["docs/testing/v1-external-uat-blockers.json"]);
  delete payload.summary.closurePhases;
...
test("fails when the external UAT blockers JSON omits next closure phase handoff metadata", () => {
  const payload = JSON.parse(completeSnapshot["docs/testing/v1-external-uat-blockers.json"]);
  delete payload.summary.nextClosurePhase;
...
test("accepts external UAT request after kickoff governance blockers close", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    "docs/testing/v1-external-uat-request.md": completeSnapshot["docs/testing/v1-external-uat-request.md"]
      .replace("Kickoff Governance\n", "")
      .replace("UAT Environment Evidence\n", "UAT Launch Intake\nUAT Environment Evidence\n")
  });

  assert.equal(result.failed.some((check) => check.id === "external-uat-request-doc"), false);
});
...
test("fails when the external UAT request tests omit stable blocker id coverage", () => {
  const snapshot = {
    ...completeSnapshot,
    "scripts/v1-external-uat-request.test.mjs": completeSnapshot["scripts/v1-external-uat-request.test.mjs"]
      .replace("exports stable machine-readable blocker ids\n", "")
  };
...
test("fails when the external UAT request tests omit blocker closure sequencing coverage", () => {
  const snapshot = {
    ...completeSnapshot,
    "scripts/v1-external-uat-request.test.mjs": completeSnapshot["scripts/v1-external-uat-request.test.mjs"]
      .replace("exports machine-readable blocker closure sequencing\n", "")
  };
...
test("fails when the external UAT request tests omit closure phase summary coverage", () => {
  const snapshot = {
    ...completeSnapshot,
    "scripts/v1-external-uat-request.test.mjs": completeSnapshot["scripts/v1-external-uat-request.test.mjs"]
      .replace("exports ordered blocker closure phase summaries\n", "")
  };
...
test("fails when the external UAT request tests omit next closure phase handoff coverage", () => {
  const snapshot = {
    ...completeSnapshot,
    "scripts/v1-external-uat-request.test.mjs": completeSnapshot["scripts/v1-external-uat-request.test.mjs"]
      .replace("exports next closure phase handoff metadata\n", "")
  };
...
test("fails when the external UAT request tests omit inconsistent Go blocker coverage", () => {
  const snapshot = {
    ...completeSnapshot,
    "scripts/v1-external-uat-request.test.mjs": completeSnapshot["scripts/v1-external-uat-request.test.mjs"]
      .replace("keeps external UAT request open when validator blockers remain despite release gate Go\n", "")
      .replace("keeps blockers JSON No-Go when validator blockers remain despite release gate Go\n", "")
      .replace("keeps closure checklist No-Go when validator blockers remain despite release gate Go\n", "")
  };
...
test("fails when the external UAT request tests omit inconsistent Go evidence intake coverage", () => {
  const snapshot = {
    ...completeSnapshot,
    "scripts/v1-external-uat-request.test.mjs": completeSnapshot["scripts/v1-external-uat-request.test.mjs"]
      .replace("keeps evidence intake No-Go when validator blockers remain despite release gate Go\n", "")
  };
...
test("fails when the kickoff governance evidence pack generator is missing", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    ".github/workflows/v1-validation.yml": completeSnapshot[".github/workflows/v1-validation.yml"]
      .replace("      - run: node --test scripts/v1-kickoff-governance-evidence-pack.test.mjs\n", "")
      .replace("      - run: node scripts/v1-kickoff-governance-evidence-pack.mjs --output docs/meeting-notes/evidence/kickoff/closure-evidence-pack.md\n", ""),
...
test("fails when the kickoff governance evidence apply helper is missing", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    ".github/workflows/v1-validation.yml": completeSnapshot[".github/workflows/v1-validation.yml"]
      .replace("      - run: node --test scripts/v1-kickoff-governance-evidence-apply.test.mjs\n", ""),
    "scripts/v1-kickoff-governance-evidence-apply.mjs": "",
    "scripts/v1-kickoff-governance-evidence-apply.test.mjs": ""
  });

  assert.equal(result.ok, false);
  assert.ok(result.failed.some((check) => check.id === "kickoff-governance-evidence-apply-helper"));
});
...
test("fails when the V1 progress TODO generator is missing", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    ".github/workflows/v1-validation.yml": completeSnapshot[".github/workflows/v1-validation.yml"]
      .replace("      - run: node --test scripts/v1-progress-todo.test.mjs\n", "")
      .replace("      - run: node scripts/v1-progress-todo.mjs --output docs/testing/v1-progress-todo.md\n", ""),
    "scripts/v1-progress-todo.mjs": "",
    "scripts/v1-progress-todo.test.mjs": ""
  });

  assert.equal(result.ok, false);
  assert.ok(result.failed.some((check) => check.id === "v1-progress-todo-generator"));
});
...
test("fails when the external UAT request tests omit evidence intake manifest ownership coverage", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    "scripts/v1-external-uat-request.test.mjs": completeSnapshot["scripts/v1-external-uat-request.test.mjs"]
      .replace("keeps evidence intake manifest ids assigned to a single intake row\n", "")
  });
...
test("fails when README omits the V1 progress TODO generation command", () => {
  const snapshot = {
    ...completeSnapshot,
    "README.md": completeSnapshot["README.md"].replace(
      "node scripts/v1-progress-todo.mjs --output docs/testing/v1-progress-todo.md\n",
...
test("fails when the V1 release gate status JSON checker omits known check id coverage", () => {
  const snapshot = {
    ...completeSnapshot,
    "scripts/v1-release-gate-status-check.mjs": completeSnapshot["scripts/v1-release-gate-status-check.mjs"]
      .replace("known-check-ids\n", ""),
    "scripts/v1-release-gate-status-check.test.mjs": completeSnapshot["scripts/v1-release-gate-status-check.test.mjs"]
      .replace("fails when the release gate JSON snapshot includes an unknown check id\n", "")
  };
...
test("fails when UAT execution materials omit the V1 progress TODO generation command", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    "docs/testing/crm-v1-test-environment-validation-runbook.md": completeSnapshot[
      "docs/testing/crm-v1-test-environment-validation-runbook.md"
    ].replace("node scripts/v1-progress-todo.mjs --output docs/testing/v1-progress-todo.md\n", "")
  });
...
test("fails when the kickoff governance intake collection form is missing", () => {
  const snapshot = { ...completeSnapshot };
  delete snapshot["docs/meeting-notes/evidence/kickoff/intake-collection-form.md"];
...
test("fails when the kickoff governance evidence pack omits task guidance", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    "docs/meeting-notes/evidence/kickoff/closure-evidence-pack.md": completeSnapshot["docs/meeting-notes/evidence/kickoff/closure-evidence-pack.md"]
      .replace("Owner Confirmation TODOList\n", "")
      .replace("Scope Freeze TODOList\n", "")
      .replace("Task Switch Display Rule\n", "")
  });
...
test("fails when the V1 progress TODO board is missing", () => {
  const snapshot = { ...completeSnapshot };
  delete snapshot["docs/testing/v1-progress-todo.md"];
...
test("fails when the V1 progress TODO board omits task switch display guidance", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    "docs/testing/v1-progress-todo.md": completeSnapshot["docs/testing/v1-progress-todo.md"]
      .replace("Task Switch Display Rule\n", "")
      .replace("当前任务\n", "")
      .replace("完成标准\n", "")
  });
  ...
  assert.equal(result.ok, false);
  assert.ok(result.failed.some((check) => check.id === "v1-progress-todo-doc"));
});

test("fails when the V1 progress TODO board omits the current task switch snapshot", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    "docs/testing/v1-progress-todo.md": completeSnapshot["docs/testing/v1-progress-todo.md"]
      .replace("Task Switch Snapshot\n", "")
      .replace("Switch readiness: `Blocked`\n", "")
      .replace("Remaining blockers before switch: 1\n", "")
  });
...
test("fails when the V1 progress TODO board omits kickoff evidence readiness", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    "docs/testing/v1-progress-todo.md": completeSnapshot["docs/testing/v1-progress-todo.md"]
      .replace("Current Task Evidence Readiness\n", "")
      .replace("Evidence templates ready: `0/14`\n", "")
      .replace("node scripts/v1-kickoff-governance-evidence-apply.mjs --decision Go --write\n", "")
  });
...
test("fails when the V1 progress TODO board omits kickoff intake readiness", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    "docs/testing/v1-progress-todo.md": completeSnapshot["docs/testing/v1-progress-todo.md"]
      .replace("Current Task Intake Readiness\n", "")
      .replace("Intake rows ready: `0/14`\n", "")
      .replace("Intake rows pending: `14`\n", "")
  });
...
test("fails when the V1 progress TODO board omits kickoff intake status command", () => {
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    "docs/testing/v1-progress-todo.md": completeSnapshot["docs/testing/v1-progress-todo.md"]
      .replace("node scripts/v1-kickoff-governance-evidence-intake.mjs --input docs/meeting-notes/evidence/kickoff/intake.json --status\n", "")
  });
...
test("accepts V1 progress TODO after switching away from kickoff governance", () => {
  const switchedProgress = `CRM V1 Progress TODO
Overall decision: \`No-Go\`
Open blockers: 41
...
`;
  ...
  const result = evaluateReadinessSnapshot({
    ...completeSnapshot,
    "docs/testing/v1-progress-todo.md": switchedProgress
  });

  assert.equal(result.failed.some((check) => check.id === "v1-progress-todo-doc"), false);
});
